...
from tools.nested_tar_dataset import NestedTarImageNet
...
class MasterAdamW:
    ...
    def step(self):
        ...
        for index, parameter in enumerate(self.params):
            ...
            parameter.update(new_master.cast(parameter.dtype))
...
def parse_args():
    ...
    parser.add_argument(
        "--data-path",
        default=os.environ.get("IMAGENET_PATH", "data/imagenet/train"),
    )
    parser.add_argument(
        "--data-tar",
        default=None,
        help="Official nested ImageNet train tar; bypasses extraction when set.",
    )
    parser.add_argument(
        "--data-index",
        default=None,
        help="NPZ offset index created by tools/nested_tar_dataset.py.",
    )
    parser.add_argument(
        "--checkpoint",
        default=None,
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "JIT_OUTPUT_DIR", "outputs/jittor_b16_training_benchmark"
        ),
    )
    ...
    parser.add_argument(
        ...
        choices=("bf16", "fp32"),
        default="bf16",
        help="Forward precision; fp32 is a compatibility fallback for CUDA stacks without BF16 atomicAdd.",
    )
    ...
    parser.add_argument(
        "--monitor-script",
        default=str(Path(__file__).resolve().parent / "tools" / "monitor_resources.py"),
    )
...
def main():
    ...
    # A single multi-GPU host runs several MPI ranks concurrently. Include the
    # rank in evidence filenames so writers never truncate one another's data.
    events_path = output_dir / f"events_{hostname}_rank{rank}.jsonl"
    resources_path = output_dir / f"resources_{hostname}_rank{rank}.csv"
    ...
    try:
        ...
        if args.data_tar:
            if not args.data_index:
                raise ValueError("--data-index is required with --data-tar")
            dataset = NestedTarImageNet(
                args.data_tar,
                args.data_index,
                transform=ImageTransform(args.image_size),
            )
            data_source = "nested_tar_index"
        else:
            dataset = ImageFolder(
                args.data_path,
                transform=ImageTransform(args.image_size),
        ...
        dataset_size = int(dataset.total_len)
        ...
        config = {
            ...
            "precision": {
                "forward_parameters_and_activations": args.precision,
        ...
        (output_dir / f"config_{hostname}_rank{rank}.json").write_text(
        ...
        for round_index in range(args.rounds):
            ...
            for step_in_round in range(args.steps_per_round):
                ...
                optimizer.lr = (
                    ...
                    * min(fractional_epoch / args.warmup_epochs, 1.0)
                )
