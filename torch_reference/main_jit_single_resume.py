...
import copy
from engine_jit_single_resume import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    parser.add_argument('--accum_iter', default=1, type=int,
                        help='Gradient accumulation steps used to preserve the effective batch size')
    parser.add_argument('--max_train_steps', default=0, type=int,
                        help='Limit micro-steps per epoch; 0 uses the complete data loader')
...
def main(args):
    ...
    if args.max_train_steps:
        assert args.max_train_steps <= len(data_loader_train)
        assert args.max_train_steps % args.accum_iter == 0
        print("Training micro-steps per epoch: %d/%d" %
              (args.max_train_steps, len(data_loader_train)))
