...
class LimitedDataLoader:
    ...
    def __iter__(self):
        return islice(iter(self.data_loader), self.limit)
...
def train_one_epoch(model, model_without_ddp, data_loader, optimizer, device, epoch, log_writer=None, args=None):
    ...
    if args.max_train_steps:
        data_loader = LimitedDataLoader(data_loader, args.max_train_steps)
    epoch_steps = len(data_loader)

    for data_iter_step, (x, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # per iteration (instead of per epoch) lr scheduler
        lr_sched.adjust_learning_rate(optimizer, data_iter_step / epoch_steps + epoch, args)
        ...
        accumulation_group_start = (data_iter_step // args.accum_iter) * args.accum_iter
        accumulation_group_size = min(
            args.accum_iter, epoch_steps - accumulation_group_start
        )
        ...
        sync_context = (
            nullcontext()
            if is_accumulation_boundary or not args.distributed
            else model.no_sync()
        )
        with sync_context:
        ...
        if log_writer is not None:
            ...
            epoch_1000x = int((data_iter_step / epoch_steps + epoch) * 1000)
